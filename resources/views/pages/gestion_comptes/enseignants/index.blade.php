<x-layout>
    ...
    <div class="container mx-auto px-4 py-8">
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
            <div>
                <h2 class="text-2xl sm:text-3xl font-bold text-gray-800">Les enseignants</h2>
                <p class="text-sm text-gray-500 mt-1">Gérez les enseignants de votre établissement</p>
            </div>
        ...
        <div class="bg-white rounded-lg shadow overflow-hidden border border-gray-200">
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200" id="enseignantsTable">
                            ...
                            @foreach ($enseignants as $enseignant)
                                <tr class="hover:bg-gray-50 transition-colors duration-150">
                                    ...
                                    {{-- <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                        {{ $enseignant->user->identifiant }}</td> --}}
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                        {{ $enseignant->prenom }}
                                        {{ $enseignant->nom }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <div class="flex justify-end space-x-2">
                                            <a href="{{ route('enseignants.matieres', $enseignant->id) }}"
                                            ...
                                            <button type="button"
                                                onclick="document.getElementById('delete_id').value = {{ $enseignant->id }}; openModal('deleteModal-{{ $enseignant->id }}')"
    ...
    @foreach ($enseignants as $enseignant)
        <!-- Modal Suppression -->
        <x-modal id="deleteModal-{{ $enseignant->id }}" title="Confirmer la suppression" maxWidth="md">
                ...
                <div class="sm:flex sm:items-start">
                    ...
                    <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                        <h3 class="text-lg leading-6 font-medium text-gray-900">
                            Supprimer l'étudiant {{ $enseignant->user->identifiant }}
                        </h3>
                        <div class="mt-2">
            ...
            <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                <form id="deleteForm" method="POST" action="{{ route('enseignants.destroy', $enseignant->id) }}">
                    @csrf
                    @method('DELETE')
                ...
                <button type="button" onclick="closeModal('deleteModal-{{ $enseignant->id }}')"
