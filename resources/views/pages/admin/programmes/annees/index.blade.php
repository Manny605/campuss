<x-layout>
                            ...
                            <tr class="hover:bg-gray-50 transition-colors duration-150">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <div class="flex justify-end space-x-2">
                                        <button
                                            onclick="document.getElementById('edit_id').value = {{ $annee->id }}; openModal('editModal-{{ $annee->id }}')"
    ...
    <x-modal id="addModal" title="Ajouter une année académique" maxWidth="lg">
        <form id="addForm" action="{{ route('programmes.storeAnnee') }}" method="POST">
            @csrf
            <x-annee-form />
    ...
    @foreach ($annees as $annee)
        ...
        <x-modal id="editModal-{{ $annee->id }}" title="Modifier une année académique" maxWidth="lg">
            <form id="editForm" method="POST" action="{{ route('programmes.updateAnnee', $annee) }}">
                @csrf
                @method('PUT')
                <input type="hidden" name="id" id="edit_id">
                <x-annee-form edit :data="$annee" />
    ...
    @foreach ($annees as $annee)
        ...
        <x-modal id="deleteModal-{{ $annee->id }}" title="Confirmer la suppression" maxWidth="md">
            ...
            <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                <form id="deleteForm" method="POST" action="{{ route('programmes.destroyAnnee', $annee->id) }}/">
                    @csrf
                    @method('DELETE')
