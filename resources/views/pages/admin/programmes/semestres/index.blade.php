<x-layout>
    ...
    <div class="container mx-auto px-4 py-8">
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
            <div>
                <h2 class="text-2xl sm:text-3xl font-bold text-gray-800">Les Semestres</h2>
                <p class="text-sm text-gray-500 mt-1">Gérez les Semestres de votre établissement</p>
            </div>
        ...
        <div class="bg-white rounded-lg shadow overflow-hidden border border-gray-200">
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200" id="semestresTable">
                        ...
                        @foreach ($semestres as $semestre)
                            <tr class="hover:bg-gray-50 transition-colors duration-150">
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                                    {{ $loop->iteration }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $semestre->code }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                    {{ $semestre->annee->libelle }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <div class="flex justify-end space-x-2">
                                        <button
                                            onclick="document.getElementById('edit_id').value = {{ $semestre->id }}; openModal('editModal-{{ $semestre->id }}')"
                                            class="text-blue-600 hover:text-blue-900 p-1.5 rounded-full hover:bg-blue-50 transition-colors duration-200"
                                            title="Modifier">
                                            <i class="fas fa-edit w-4 h-4"></i>
                                        </button>
                                        <button
                                            onclick="document.getElementById('delete_id').value = {{ $semestre->id }}; openModal('deleteModal-{{ $semestre->id }}')"
    ...
    <!-- Modals -->
    <!-- Modal Ajout -->
    <x-modal id="addModal" title="Ajouter un semestre" maxWidth="lg">
        <form id="addForm" action="{{ route('programmes.storeSemestre') }}" method="POST">
            @csrf
            <x-semestre-form  :annees="$annees" />
    ...
    @foreach ($semestres as $semestre)
        <!-- Modal Édition -->
        <x-modal id="editModal-{{ $semestre->id }}" title="Modifier un semestre" maxWidth="lg">
            <form id="editForm" method="POST" action="{{ route('programmes.updateSemestre', $semestre) }}">
                @csrf
                @method('PUT')
                <input type="hidden" name="id" id="edit_id">
                <x-semestre-form edit :data="$semestre" :annees="$annees" />

                <div class="mt-6 flex justify-end space-x-3">
                    <button type="button" onclick="closeModal('editModal-{{ $semestre->id }}')"
                        class="px-4 py-2 border border-gray-300 rounded-lg shadow-sm text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-colors duration-200">
                        Annuler
                    </button>
                    <button type="submit"
    ...
    @foreach ($semestres as $semestre)
        <!-- Modal Suppression -->
        <x-modal id="deleteModal-{{ $semestre->id }}" title="Confirmer la suppression" maxWidth="md">
                    ...
                    <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                        ...
                        <div class="mt-2">
                            <p class="text-sm text-gray-500">
                                Êtes-vous sûr de vouloir supprimer ce semestre ? Cette action est
                                irréversible.
                            </p>
                        </div>
                    </div>
            ...
            <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                <form id="deleteForm" method="POST" action="{{ route('programmes.destroySemestre', $semestre->id) }}/">
                    @csrf
                    @method('DELETE')
                ...
                <button type="button" onclick="closeModal('deleteModal-{{ $semestre->id }}')"
    ...
    <script>
        ...
        function filterTable() {
            ...
            const rows = document.querySelectorAll("#semestresTable tbody tr");
