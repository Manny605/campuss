<x-layout>
    ...
    <x-modal id="addModal" title="Ajouter une filiere" maxWidth="lg">
        <form id="addForm" action="{{ route('programmes.storeFiliere') }}" method="POST">
            @csrf
            <x-filiere-form />
    ...
    @foreach ($filieres as $filiere)
        ...
        <x-modal id="editModal-{{ $filiere->id }}" title="Modifier une filiere" maxWidth="lg">
            <form id="editForm" method="POST" action="{{ route('programmes.updateFiliere', $filiere) }}">
                @csrf
                @method('PUT')
                <input type="hidden" name="id" id="edit_id">
                <x-filiere-form edit :data="$filiere" />
