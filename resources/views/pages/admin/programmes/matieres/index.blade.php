<x-layout>
    ...
    <div class="container mx-auto px-4 py-8">
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
            <div>
                <h2 class="text-2xl sm:text-3xl font-bold text-gray-800">Les Matieres</h2>
                <p class="text-sm text-gray-500 mt-1">Gérez les matieres de votre établissement</p>
            </div>
        ...
        <div class="bg-white rounded-lg shadow overflow-hidden border border-gray-200">
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200" id="matieresTable">
                        ...
                        @foreach ($matieres as $matiere)
                            <tr class="hover:bg-gray-50 transition-colors duration-150">
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                                    {{ $loop->iteration }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $matiere->code }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $matiere->nom }}</td>
    ...
    @foreach ($matieres as $matiere)
        <!-- Modal Édition -->
        <x-modal id="editModal-{{ $matiere->id }}" title="Modifier une matiere" maxWidth="lg">
