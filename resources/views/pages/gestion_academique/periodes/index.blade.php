<x-layout>
                            ...
                            <tr class="hover:bg-gray-50 transition-colors duration-150">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <div class="flex justify-end space-x-2">
                                        ...
                                        <button
                                            onclick="document.getElementById('delete_id').value = {{ $periode->id }}; openModal('deleteModal-{{ $periode->id }}')"
    ...
    <!-- Modals -->
    <!-- Modal Ajout -->
    <x-modal id="addModal" title="Ajouter un periode" maxWidth="lg">
    ...
    @foreach ($periodes as $periode)
        <!-- Modal Édition -->
        <x-modal id="editModal-{{ $periode->id }}" title="Modifier une periode" maxWidth="lg">
    ...
    @foreach ($periodes as $periode)
        <!-- Modal Suppression -->
        <x-modal id="deleteModal-{{ $periode->id }}" title="Confirmer la suppression" maxWidth="md">
                ...
                <div class="sm:flex sm:items-start">
                    ...
                    <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                        <h3 class="text-lg leading-6 font-medium text-gray-900">
                            Supprimer le periode
                        </h3>
                        <div class="mt-2">
                            <p class="text-sm text-gray-500">
                                Êtes-vous sûr de vouloir supprimer cette periode ? Cette action est
                                irréversible.
                            </p>
                        </div>
                    </div>
            ...
            <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                <form id="deleteForm" method="POST" action="{{ route('periodes.destroy', $periode->id) }}/">
                    @csrf
                    @method('DELETE')
